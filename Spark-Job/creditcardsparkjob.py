...
spark = SparkSession.builder \
    .appName("Credit Card Transactions Processing") \
    .getOrCreate()

# Daily transactions data path
json_file_path = 'gs://credit-card-data-processing/transactions/transactions_*.json'


# Define BigQuery Dataset & Table Names
BQ_PROJECT_ID = #"<GCP Project ID>" 
BQ_DATASET = "credit_card_data_analysis"
...
'''
-Filter transactions based on basic data quality checks:
-Ensure transaction amount is non-negative.
-Validate transaction status against allowed values.
- Check for the presence of cardholder and merchant IDs.
'''
